...
contract Domains is ERC721URIStorage {
    ...
    constructor(string memory _tld) payable ERC721("Lizard Name Service", "LNS") {
        tld = _tld;
    ...
    function register(string calldata name) public payable {
        ...
        require(msg.value >= _price, 'Not enought Matic paid');

        string memory _name = string(abi.encodePacked(name, ".", tld));
        string memory finalSvg = string(abi.encodePacked(svgPartOne, _name, svgPartTwo));
        uint256 newRecordId = _tokenIds.current();
        uint256 lenght = StringUtils.strlen(name);
        string memory strLen = Strings.toString(lenght);
        ...
        string memory finalTokenUri = string( abi.encodePacked("data:aplication/json;base64,", json));
